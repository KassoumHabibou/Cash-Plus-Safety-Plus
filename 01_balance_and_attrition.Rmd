...
label_vec = c("Benef. is HH head",
              ...
              "Literate (hh. head)",
              "Literate (benef.)",
              # "Health index (HH head)",
              # "Health index (beneficiary)",
...
# stubs <- c("coach", "avec", "video", "germe", "acv", "grant")
# 
# #coach : coach visits
# #avec : vsla
...
# 
# compliance_rates <- purrr::map_dfr(stubs, function(stub){ 
#     # if(stub %in% c("coach", "avec", "germe")) {
#     #   filtered_data <- followup_MRT_hh %>% filter(treatment %in% c(1, 2, 3))
#     # } else if (stub %in% c("video", "acv")) {
#     #   filtered_data <- followup_MRT_hh %>% filter(treatment %in% c(1, 3))
#     #   } else if (stub == "bourse") {
#     # filtered_data <- followup_MRT_hh %>% filter(treatment %in% c(2, 3))
#     # }
#   
#   #Overall mean on every one
#   overall_mean <- round(mean(followup_MRT_hh[[paste0("got_", stub)]], na.rm = TRUE),3) * 100
#   
#     temp_balance <- followup_MRT_hh %>% 
#       split(.$treatment) %>%
#       purrr::map_dfr(., function(splitted_df){
#         
#         treatment_mean <- round(mean(splitted_df[[paste0("got_", stub)]], na.rm = TRUE),3) * 100
#         
#   })
#     
#     temp_balance <- tibble(
...
## Cross comparaison (Tekavoul and the PI)
