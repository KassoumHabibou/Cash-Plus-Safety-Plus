...
![Geographic coverage of the Economic inclusion pilot and impact evaluation in Mauritania.](~/Dropbox/PhD thesis/chapitre_5/output/img/CoveragePI.png)
...
# Convert attrition_tab_2 to a named vector for add_stat()
coef_vector <- setNames(attrition_tab_2$`(1)-(2)`, var_vec)

# Add the coefficient column to the gtsummary table
attrition_tab <- cbind(
  as_tibble(attrition_tab_1),
  as_tibble(attrition_tab_2))
...
label_vec = c("Benef. is HH head",
              ...
              "Primary education (0/1, H-partner)",
...
followup_MRT_hh <- readRDS(paste0(getwd(),"/output/data/followup_ipv_MRT_hh.rds"))
...
cross_tab <- cross_tab %>% 
  ...
  rename("Tekavoul" = Mean_CSH, "Economic Inclusion" = Mean_PI, "Diff."=coef_val) %>% 
  select(Variable, Tekavoul, `Economic Inclusion`, `Diff.`)
